...
public class ScreenManager
{
	...
	public void Update( float frameTime )
	{
		...
		if ( d_ShowSafeArea )
		{
			...
			_UI.Sprite.AddSprite( 0, _UI.Sprite.TopLayer, _UI.SXL, _UI.SYT, 0.0f, _UI.SSX, _UI.SSY, 0.0f, E_Align.TopLeft, ref c, ref renderState );
	...
	// AddMessageNow
	public void AddMessageNow( int type, int data )
	{
		ScreenMessage message = new ScreenMessage( type, data );

		int oldCurrentScreenControl = CurrentScreenControl;

		for ( int i = 0; i < ScreenControls.Count; ++i )
		{
			CurrentScreenControl = i;
			ScreenControls[ i ].ProcessMessage( ref message );
		}

		CurrentScreenControl = oldCurrentScreenControl;
	}

	public void AddMessageNow( int type )
	{
		AddMessageNow( type, -1 );
	}
	...
	// ShutdownAllButCurrent
	public void ShutdownAllButCurrent()
	{
		for ( int i = 0; i < ScreenControls.Count; ++i )
		{
			if ( i == CurrentScreenControl )
				continue;

			ScreenControl screenControl = ScreenControls[ i ];

			screenControl.EndTimer = 0.0f; // shutdown straight away
			screenControl.SetNextScreen( null );
		}
	}
