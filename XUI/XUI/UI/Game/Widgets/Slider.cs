...
public class WidgetSlider : WidgetBase
{
	...
	protected override void OnRender()
	{
		...
		_UI.Sprite.AddSprite( RenderPass, Layer, ref pos, ref Size, E_Align.TopLeft, ref ColorFinal, ref RenderState );
		_UI.Sprite.AddTexture( 0, Textures[ 0 ] );

		float pos01 = (float)( Value - MinValue ) / ( MaxValue - MinValue );

		_UI.Sprite.AddSprite( RenderPass, Layer, pos.X + ( Size.X * pos01 ), pos.Y + ( Size.Y / 2.0f ) * UpY, pos.Z, IndicatorSize.X, IndicatorSize.Y, 0.0f, E_Align.MiddleCentre, ref ColorFinal, ref RenderState );
