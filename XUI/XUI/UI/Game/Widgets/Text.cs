...
public class WidgetText : WidgetBase
{
	...
	protected override void CopyTo( WidgetBase o )
	{
		...
		oo.DoFixedWidthNumbers = DoFixedWidthNumbers;
	...
	{
		if ( ( RenderSizeY <= 0.0f ) || ( !ColorFinal.HasAlpha() && ( ( RenderState.BlendState == E_BlendState.AlphaBlend ) || ( RenderState.BlendState == E_BlendState.NonPremultiplied ) ) ) )
			return;
		...
		if ( DoFixedWidthNumbers )
			_UI.Font.DoFixedWidthNumbersNext();
		...
		else
		{
			...
			if ( ( StringUI != null ) && ( StringUI.Length != 0 ) )
	...
	public  bool					DoFixedWidthNumbers;

	public  float					RenderSizeY		{ get; private set; }
	public  float					RenderSizeX		{ get; private set; }
