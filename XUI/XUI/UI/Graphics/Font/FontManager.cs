...
public class FontManager
{
	...
	// DoFixedWidthNumbersNext
	public void DoFixedWidthNumbersNext()
	{
		DoFixedWidthNumbers = true;
	}
	...
	private unsafe Vector2 Process( ref StringProxy text, FontStyle fontStyle, float height, float wrapSizeX, E_FontProcessType processType )
	{
		...
		float numberWidthScaled = fontData.NumberWidth * scale;
		...
		fixed ( FontDataChar* pCharDataStart = &fontData.Characters[ 0 ] )
		{
			...
			for ( int i = 0; i < text.Length; ++i )
			{
				...
				else
				{
					...
					if ( ( ( i + 5 ) <= text.Length ) && ( c == '[' ) && ( s[ i + 1 ] == '[' ) )
					{
						...
						{
							// valid format
							fontIconIndex = _UI.Store_FontIcon.Get( StringUI );
							...
							if ( fontIconIndex != -1 )
							{
								...
								pSprite->ActualSizeX = pSprite->Size.X;
					...
					if ( fontIconIndex == -1 )
					{
						...
						bool fixedWidth = ( DoFixedWidthNumbers && ( c >= '0' ) && ( c <= '9' ) );

						pSprite->Size.X = pCharData->Width * scale;
						pSprite->Size.Y = pCharData->Height * scale;

						pSprite->Position.X = x + ( fixedWidth ? ( ( numberWidthScaled / 2.0f ) - ( pSprite->Size.X / 2.0f ) ) : ( pCharData->OffsetX * scale ) );
						pSprite->Position.Y = y + ( pCharData->OffsetY * scale ) * multY;
						pSprite->Position.Z = Position.Z;

						pSprite->ActualSizeX = ( fixedWidth ? ( numberWidthScaled - ( pSprite->Position.X - x ) ) : pSprite->Size.X );
						...
						// advance
						x += ( fixedWidth ? numberWidthScaled : ( pCharData->AdvanceX * scale ) );

						// kerning
						if ( !fixedWidth && fontData.HasKerning && ( i != ( text.Length - 1 ) ) )
				...
				if ( ( Align != E_Align.TopLeft ) && ( Align != E_Align.None ) )
				{
					...
					else
					{
						...
						for ( int i = 0; i < SpriteList.RenderEnd; ++i, ++pSpriteLineEnd )
						{
							...
							float lineWidth = ( pSpriteLineEnd->Position.X + pSpriteLineEnd->ActualSizeX ) - startX;
			...
			// reset
			DoFixedWidthNumbers = false;

			return new Vector2( ( numLines == 1 ) ? ( x - startX ) : wrapSizeX, totalHeight ); // TODO - this is (potentially) bugged in x if you use manual line breaks.
		}
	}
	...
	private bool					DoFixedWidthNumbers;
