...
public class FontStyle
{
	...
	// GetRenderPass
	public FontStyleRenderPass GetRenderPass( int index )
	{
			...
			for ( int i = 0; i < RenderPasses.Count; ++i )
			{
				...
				for ( int j = list.RenderStart; j < list.RenderEnd; ++j, ++pSprite )
				{
					...
					if ( isIcon )
					{
						...
						_UI.Sprite.AddSprite( list.RenderPass, list.Layer + fontIcon.LayerOffset, position.X, position.Y, position.Z, pSprite->Size.X, pSprite->Size.Y, 0.0f, E_Align.TopLeft, ref colors, ref fontIcon.RenderState );
					}
					else
						_UI.Sprite.AddSprite( list.RenderPass, list.Layer, position.X, position.Y, position.Z, pSprite->Size.X, pSprite->Size.Y, 0.0f, E_Align.TopLeft, ref colors, ref RenderState );
