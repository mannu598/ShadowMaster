...
//
public delegate void SpriteManager_CustomRender( ref Matrix viewProj, ref Sprite sprite, ref Matrix m, object userData );
//
...
public class SpriteManager
{
	...
	public SpriteManager()
	{
		...
		UserDataPool = new object[ _UI.Settings.Sprite_UserDataCount ];
		UserDataCount = 0;
	...
	// StoreUserData
	public int StoreUserData( object userData )
	{
		if ( UserDataCount == _UI.Settings.Sprite_UserDataCount )
			return -1;

		int count = UserDataCount++;
		UserDataPool[ count ] = userData;
		return count;
	}
	...
	public unsafe void AddSprite( int renderPass, int layer, BlendState blendState, bool pushState )
	{
		...
		{
			pSprite->Type = (int)E_SpriteType.BlendState;
	...
	public unsafe void AddSprite( int renderPass, int layer, DepthStencilState depthStencilState, int stencilRef, bool pushState )
	{
		...
		fixed ( Sprite* pSprite = &Sprites[ SpriteCount ] )
		{
			pSprite->Type = (int)E_SpriteType.DepthStencilState;
	...
	public unsafe void AddSprite( int renderPass, int layer, RasterizerState rasterizerState, ref Vector2 scissorPosition, ref Vector2 scissorSize, bool pushState )
	{
		...
		fixed ( Sprite* pSprite = &Sprites[ SpriteCount ] )
		{
			pSprite->Type = (int)E_SpriteType.RasterizerState;
	...
	public void AddSprite( int renderPass, int layer, ref Vector3 pos, ref Vector3 size, E_Align align, ref SpriteColors colors, ref RenderState renderState )
	{
		AddSprite( renderPass, layer, ref pos, ref size, align, ref colors, ref renderState, (int)E_SpriteType.Sprite );
	}

	public unsafe void AddSprite( int renderPass, int layer, ref Vector3 pos, ref Vector3 size, E_Align align, ref SpriteColors colors, ref RenderState renderState, int spriteType )
	{
		if ( SpriteCount == SpriteCountMax )
			return;
		...
		fixed ( Sprite* pSprite = &Sprites[ SpriteCount ] )
		{
			...
			pSprite->Size.X = size.X;
			pSprite->Size.Y = size.Y;
			pSprite->Size.Z = size.Z;
	...
	public void AddSprite( int renderPass, int layer, float pX, float pY, float pZ, float sX, float sY, float sZ, E_Align align, ref SpriteColors colors, ref RenderState renderState )
	{
		Vector3 p = new Vector3( pX, pY, pZ );
		Vector3 s = new Vector3( sX, sY, sZ );

		AddSprite( renderPass, layer, ref p, ref s, align, ref colors, ref renderState );
	}
	...
	// AddUserData
	public unsafe void AddUserData( int index )
	{
		fixed ( Sprite* pSprite = &Sprites[ CurrentSprite ] )
			pSprite->UserDataIndex = index;
	}
	...
	private unsafe void RenderLayer( int renderPass, List< int > sprites )
	{
		...
		fixed ( VertexSprite* pVertexDataStart = &VertexData[ 0 ] )
		{
			for ( int i = 0; i < sprites.Count; )
			{
				...
				if ( pSprite->Type >= (int)E_SpriteType.Count )
				{
					Matrix m = Matrix.Identity;

					if ( pSprite->PreTransformIndex != 0 )
						Matrix.Multiply( ref m, ref PreMatrixPool[ pSprite->PreTransformIndex ], out m );

					m.Translation += pSprite->Position;

					if ( pSprite->PostTransformIndex != 0 )
						Matrix.Multiply( ref m, ref PostMatrixPool[ pSprite->PostTransformIndex ], out m );
					
					if ( SpriteManager_CustomRender != null )
					{
						SetBlendState( pSprite );
						SpriteManager_CustomRender( ref TransformMatrix, ref *pSprite, ref m, ( ( pSprite->UserDataIndex == -1 ) ? null : UserDataPool[ pSprite->UserDataIndex ] ) );
					}
				...
				if ( pSprite->Type == (int)E_SpriteType.BlendState )
				{
					if ( pSprite->PreTransformIndex != 0 )
					{
				...
				if ( pSprite->Type == (int)E_SpriteType.DepthStencilState )
				{
					if ( pSprite->PreTransformIndex != 0 )
					{
				...
				if ( pSprite->Type == (int)E_SpriteType.RasterizerState )
				{
					if ( pSprite->PreTransformIndex != 0 )
					{
	...
	// SetBlendState
	private unsafe void SetBlendState( Sprite* pSprite )
	{
	...
	public  event SpriteManager_CustomRender	SpriteManager_CustomRender;
	//
};
