...
public class Screen_LevelSelect : Screen
{
		...
		: base( "LevelSelect" )
	{
        ...
        background0.ColorBase = Color.White;
        ...
        for ( int i = 0; i < 4; ++i )
		{
			...
			for ( int j = 0; j < numRows; ++j )
			{
				for ( int k = 0; k < numCols; ++k )
				{
                    ...
                    graphic.AddTexture("levelselect_board", 0.0f, 0.0f, 1.0f, 1.0f);
                    graphic.Parent( node );
					graphic.ParentAttach = E_Align.MiddleCentre;
					graphic.ColorBase = Color.White;

                    WidgetGraphic shadow = new WidgetGraphic();
                    shadow.RenderPass = 1;
                    shadow.Layer = 3;
                    shadow.Position = new Vector3(0.0f, 0.0f, 0.0f);
                    shadow.Size = new Vector3(6.0f, 6.0f, 0.0f);
                    shadow.Alpha = 0.5f;
                    shadow.AddTexture("shadow" + (i + 1).ToString() + "_" + (j * 3 + k + 1).ToString(), 0.0f, 0.0f, 1.0f, 1.0f);
                    shadow.ColorBase = Color.White;
                    shadow.Align = E_Align.MiddleCentre;
                    shadow.ParentAttach = E_Align.MiddleCentre;
                    shadow.Parent(graphic);

                    if (SavingDataHandler.isLevelCompleted[i, j * 3 + k] == 0)
					{
                    ...
                    if (SavingDataHandler.isLevelCompleted[i, j*3+k] != 0)
					{
                        ...
                        if (SavingDataHandler.isLevelCompleted[i, j * 3 + k] == 1)
                        {
                            tick.AddTexture("star_bronze", 0.0f, 0.0f, 1.0f, 1.0f);
                        }
                        else if (SavingDataHandler.isLevelCompleted[i, j * 3 + k] == 2)
                        {
                            tick.AddTexture("star_silver", 0.0f, 0.0f, 1.0f, 1.0f);
                        }
                        else if (SavingDataHandler.isLevelCompleted[i, j * 3 + k] == 3)
                        {
                            tick.AddTexture("star_gold", 0.0f, 0.0f, 1.0f, 1.0f);

                            WidgetGraphic mastered = new WidgetGraphic();
                            mastered.RenderPass = 1;
                            mastered.Layer = 3;
                            mastered.Position = new Vector3(-1.5f, 1.5f, 0.0f);
                            mastered.Size = new Vector3(3.0f, 3.0f, 0.0f);
                            mastered.Align = E_Align.TopLeft;
                            mastered.Parent(graphic);
                            mastered.ParentAttach = E_Align.TopLeft;
                            mastered.ColorBase = Color.White;
                            mastered.AddTexture("mastered", 0.0f, 0.0f, 1.0f, 1.0f);
                            Add(mastered);
			...
			textWorld.String = "LEVEL " + ( i + 1 );
            
            switch (i)
            {
                case 0:
                    textWorld.String += ": SHADOW KID";
                        break;
                case 1:
                        textWorld.String += ": SHADOW PERFORMER";
                        break;
                case 2:
                        textWorld.String += ": SHADOW MASTER";
                        break;
                case 3:
                        textWorld.String += ": SHADOW COUPLE";
                        break;
            }
			...
			for ( int j = 0; j < LevelNodes[ i ].Count; ++j )
                if (SavingDataHandler.isLevelCompleted[i, j] == 3)
					++count;
            ...
            if (count == LevelNodes[i].Count)
            {
                ...
                tickWorld.AddTexture("tick", 0.0f, 0.0f, 1.0f, 1.0f);
	...
	protected override void OnProcessInput( Input input )
    {
        ...
        if (Mode == 1)
        {
            if (_G.GameInput.Mouse.X() < 235 || _G.GameInput.Mouse.X() > 1045 || _G.GameInput.Mouse.Y() < 118 || _G.GameInput.Mouse.Y() > 591)
                if (_G.Mouse_Click())
                {
                    _G.Switch.Play();
                    Mode = 0;
        ...
        if (Mode == 0)
        {
            ...
            if (_G.GameInput.Mouse.ScrolledDown())
            {
                if (Menu.GetByValue() < 4)
                    Menu.SetByValue(Menu.GetByValue() + 1);
            }
            else if (_G.GameInput.Mouse.ScrolledUp())
            {
                if (Menu.GetByValue() > 0)
                    Menu.SetByValue(Menu.GetByValue() - 1);
            }
            if (_G.GameInput.Mouse.ButtonJustReleased((int)E_MouseButton.Left) && _G.GameInput.Mouse.DX() < -5 )
            {
        ...
        if (Mode == 1)
        {
            ...
            // 鼠标选择关卡
            //Console.WriteLine(_G.GameInput.Mouse.XY().ToString());
            if (_G.GameInput.Mouse.X() > 235 && _G.GameInput.Mouse.X() < 491)
            {
                if (_G.GameInput.Mouse.Y() > 118 && _G.GameInput.Mouse.Y() < 264)
                    CurrentSelectionLevel = 0;
                if (_G.GameInput.Mouse.Y() > 291 && _G.GameInput.Mouse.Y() < 430)
                    CurrentSelectionLevel = 3;
                if (_G.GameInput.Mouse.Y() > 457 && _G.GameInput.Mouse.Y() < 591)
                    CurrentSelectionLevel = 6;
            }
            else if (_G.GameInput.Mouse.X() > 528 && _G.GameInput.Mouse.X() < 760)
            {
                if (_G.GameInput.Mouse.Y() > 118 && _G.GameInput.Mouse.Y() < 264)
                ...
                if (_G.GameInput.Mouse.Y() > 457 && _G.GameInput.Mouse.Y() < 591)
                    CurrentSelectionLevel = 7;
            }
            else if (_G.GameInput.Mouse.X() > 793 && _G.GameInput.Mouse.X() < 1045)
            {
                if (_G.GameInput.Mouse.Y() > 118 && _G.GameInput.Mouse.Y() < 264)
                    CurrentSelectionLevel = 2;
                if (_G.GameInput.Mouse.Y() > 291 && _G.GameInput.Mouse.Y() < 430)
                    CurrentSelectionLevel = 5;
                if (_G.GameInput.Mouse.Y() > 457 && _G.GameInput.Mouse.Y() < 591)
                    CurrentSelectionLevel = 8;
            } 
            ...
            if (input.ButtonJustPressed((int)E_UiButton.A) || _G.Mouse_Click())
            {
                _G.ButtonPressed.Play();
                _G.gameLevel = CurrentSelectionWorld + 1;
                _G.gameNum = CurrentSelectionLevel + 1;
	...
	//
    private static string[] TextureNames = { "levelselect_bg", "levelselect_bg", "levelselect_bg", "levelselect_bg" };
